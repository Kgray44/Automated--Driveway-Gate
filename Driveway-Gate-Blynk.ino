...
void loop() {
  ...
  if (gpsSelect != 1){
    Blynk.virtualWrite(V7,calcDist(GPSLat, GPSLong, MY_GATE_LAT, MY_GATE_LON));
    if (comingHome == 1){
      Serial.println(calcDist(GPSLat, GPSLong, MY_GATE_LAT, MY_GATE_LON));
      if (calcDist(GPSLat, GPSLong, MY_GATE_LAT, MY_GATE_LON) < distanceSelect){
